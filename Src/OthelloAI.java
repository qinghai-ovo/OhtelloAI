...
class OthelloAI{
    ...
    public record BestPosR(Point point, int value) {}
    public static BestPosR getBestPos(Map<Point, Integer> legalPos,int[][] board, int my,int layer){
        ...
        for (Map.Entry<Point, Integer> entry : legalPos.entrySet()) {
            ...
            if (layer == 0) {
                currentScore = weightMatrix[entry.getKey().x][entry.getKey().y] + entry.getValue();
            }else{
                Map<Point, Integer> opPoss = getLegalPos(newBoard, op);
                BestPosR opRes = getBestPos(opPoss, newBoard, op, layer - 1);
                int opMax = opRes.value();
                 //System.out.println("("+entry.getKey()+") "+ opMoveNum);
                currentScore = weightMatrix[entry.getKey().x][entry.getKey().y] + entry.getValue() - opMax;
            }
        ...
        return new BestPosR(bestPos, bestScore);
    }
        ...
        try {
            ...
            while(true){
                ...
                if(serverResponse.startsWith("BOARD ")){
                    ...
                    Point putPos =  getBestPos(legalPos, board, mycolor,5).point();
